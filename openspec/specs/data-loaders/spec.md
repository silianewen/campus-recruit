...
Defines the frontend data-loading pattern: loaders that read from Supabase, the `useAsync` hook that wraps them, and the `AsyncView` component that renders loading / error / empty / data states. Establishes that company, position, and skill-question data come from the DB at runtime rather than hardcoded registries, and that the upload flow's data shape (resumes + submissions with company_id and position_id) is preserved.
