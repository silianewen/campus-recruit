...
Defines the dark-mode theme: a user-visible toggle on every page, persistence in `localStorage`, no flash-of-wrong-theme on initial render, per-device preferences (no server-side sync), and accessibility (keyboard and screen-reader support).
