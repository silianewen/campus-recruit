...
export default function Personality() {
  ...
  if (allDone && result) {
    ...
    return (
      <Page title="你的测评结果">
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-8 max-w-2xl mx-auto text-center">
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-2">你的 MBTI 类型是</p>
          <div className="text-6xl font-bold text-blue-600 dark:text-blue-400 mb-2">{result.type}</div>
          <div className="text-xl text-slate-700 dark:text-slate-300 mb-4">{desc?.name ?? '独特个性'}</div>
          <p className="text-slate-600 dark:text-slate-300 mb-6 max-w-md mx-auto">{desc?.summary ?? '继续保持真我。'}</p>

          <div className="grid grid-cols-4 gap-2 max-w-sm mx-auto mb-6 text-xs">
            {(['E', 'I', 'S', 'N', 'T', 'F', 'J', 'P'] as const).map((k) => (
              <div key={k} className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-2">
                <div className="font-bold text-slate-900 dark:text-slate-100">{k}</div>
                <div className="text-slate-500 dark:text-slate-400">{result.scores[k]}/5</div>
              </div>
            ))}
          </div>

          {error && <p className="text-red-600 dark:text-red-400 text-sm mb-3">{error}</p>}
          {submitting && <p className="text-xs text-blue-600 dark:text-blue-400 mb-3">保存中…</p>}
          {saved && !error && (
            <p className="text-xs text-emerald-600 dark:text-emerald-400 mb-3">✅ 已保存到 HR 后台</p>
          )}

          <div className="flex gap-2 justify-center">
            <button onClick={() => navigate('/status')} className="px-4 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-900 dark:text-slate-100 rounded-lg text-sm">
              查看我的全部状态
            </button>
          ...
          <button onClick={() => void submit()} disabled={submitting}
            className="mt-3 text-xs text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 disabled:opacity-50">
  ...
  return (
    <Page title="性格测评" subtitle={`第 ${step + 1} / ${total} 题`}>
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 max-w-xl mx-auto">
        ...
        <div className="text-xs text-slate-400 dark:text-slate-500 mb-1">{current.dimension} 维度</div>
        <h2 className="text-lg font-medium text-slate-900 dark:text-slate-100 mb-5">{current.text}</h2>
