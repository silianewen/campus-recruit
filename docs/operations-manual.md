...
### 3.5 HR 后台 8 个新能力（2026-07-19 上线）

| 优先级 | 能力 | 怎么用 |
|---|---|---|
| P1 | **发通知** | 列表每行末尾"通知"按钮 → 弹窗填标题+类型+内容 → 学生 `/status` 看到 |
| P2 | **岗位截止时间** | 学生在 `closes_at` 之后看到"该岗位已于 XXXX 截止"页，无法投递；列表仍可见 |
| P3 | **CSV 导出** | 列表顶部"导出 CSV"按钮 → 浏览器下载当前筛选结果（UTF-8 BOM，Excel 直开） |
| P4 | **移动端适配** | 360px 视口下：列表变卡片堆叠、看板 5 图 1 列堆叠 |
| P5 | **简历下载** | 列表每行"下载"按钮 → 浏览器保存原文件名 |
| P6 | **编辑账号** | `/hr/admin/users` 每行"编辑"按钮 → 改用户名/显示名/密码（不能改自己） |
| P7 | **隐藏默认密码** | 登录页不再显示 "admin / Admin@2026" 提示（凭据仍有效，文档有） |
| P8 | **公司组看板只看自家** | 昶联/中南机诚等公司组用户登录后，看板 5 个图全部只显示自家数据 |

详细使用流程见下面各小节。

### 3.5a 发面试通知（P1）

进 `/hr/list` → 找到目标学生那行 → 点行尾"通知"按钮 → 弹窗：
- 标题：默认"面试通知"，可改
- 类型：面试邀请 / 测评邀请 / 状态更新
- 内容：默认模板可改
- 点"发送" → 学生侧 `/status` 立刻看到 🔔 未读通知
- 若类型是"面试邀请"且状态不是"已约面"，自动把状态改为"已约面"

> 默认分组（只读）看不到"通知"按钮。

### 3.5b 导出 CSV（P3）

`/hr/list` 顶部"导出 CSV"按钮：

- 文件名：`投递简历_2026-07-19_昶联金属_已约面.csv`（包含日期 + 当前筛选）
- 列：12 列（11 表列 + 1 "其它投递"列；重复投递时显示其它公司+岗位）
- 字符集：UTF-8 with BOM，Excel / WPS 直接打开不乱码
- 容量上限：1000 条。超过会弹"筛选结果过大，请缩小范围"提示

### 3.5c 编辑账号（P6）

`/hr/admin/users`（仅集团管理员可见）：

- 每行"编辑"按钮 → 弹窗
- 3 个字段：用户名 / 显示名 / 新密码（留空 = 保持原密码）
- 不能编辑自己（按钮显示"（自己）"占位）
- 用户名冲突时弹"用户名已存在"

### 3.5d 公司组看板严格只显示自家（P8）

`/hr/dashboard` 登录：

- **集团管理员**（`admin`）：看到全部 5 家公司 + 全部 10 个岗位
- **公司组**（如 `changlian_metal`）：只看到昶联金属 1 家公司 + 昶联的 2 个岗位（人力行政、采购）+ 昶联学生的数据
- **默认分组**（`default`）：看到全部（只读）

5 个图（按职位 / 按公司 / 按学历 / 专业 Top 10）的内容都按上述规则过滤。

### 3.5e 其他新能力快速参考

- **P2 截止时间**：`/companies/:id` 岗位卡片显示 "X 天后截止" 徽章（14 天内）或 "已截止"（过期）；过期时该卡片变灰且不可点
- **P4 移动端**：用手机访问 https://campusrecruitment.vercel.app/ 验证
- **P5 简历下载**：列表每行"查看"（新标签打开）/ "下载"（保存到本地）两个按钮
- **P7 默认密码**：登录页不再显示任何凭据提示，凭据只在 `docs/operations-manual.md` §3.1.1 中

### 3.5f 给学生发面试通知（通过改状态 + 通知）
